...
class Shift(models.Model):
    ...
    def get_pending_vehicles(self):
        ...
        return Vehicle.objects.filter(exit_time__isnull=True, is_active=True)

    def get_vehicles_in_shift(self):
        """Obtener vehículos que entraron durante este turno"""
        return Vehicle.objects.filter(entry_time__gte=self.open_time, entry_time__lte=self.close_time, is_active=True)
